...
public partial class Visitor_CheckOut : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        strSql = "select BookName,Price from tb_Book where BookID=" + Convert.ToInt32(drRow["BookID"].ToString());
                       // dstable = dbObj.GetDataSetStr(strSql, "tbGI");
                        dstable = ope.SelectTable(strSql,"tb_Book").Tables["tb_Book"];
                        drRow["No"] = i;
    ...
    protected void btnConfirm_Click(object sender, EventArgs e)
    {
        ...
        //{
            //得到用户输入的信息
        string strPhone = this.txtReceiverPhone.Text.Trim();  //电话号码
          ...
          string strTotalShipFee=  Convert.ToString(fltTotalShipFee);
        string strShipType=ddlShipType.SelectedItem.Text;
        string strBTotalPrice=labTotalPrice.Text;
           // string strMID=Session["UID"].ToString();
            //将订单信息插入订单表中
           ...
           // string sql = "insert into tb_Order OrderDate='" + DateTime.Now + "' ReceiverName='" + strName + "' ReceiverPhone='" + strPhone + "' ReceiverAddress='" + strAddress + "' MemberID=" + 1 + "' ReMark='" + strRemark + "' ShipType='" + this.ddlShipType.SelectedItem.Text + "' ShipFee='" + fltTotalShipFee + "' BTotalPrice=" + labTotalPrice.ToString() + ";SELECT @@Identity";
           // string sql = "insert into tb_Order (ReceiverName,ReceiverPhone,ReceiverAddress,MemberID,ReMark,ShipType,ShipFee,BTotalPrice) values ("+strName+","+strPhone+","+strAddress+","+"1"+","+strRemark+","+strShipType+","+strTotalShipFee+","+labTotalPrice.ToString()+");select @@Identity";   
      //string sql = "insert into tb_Order (ReceiverName,ReceiverPhone,ReceiverAddress,MemberID,ReMark,ShipType,ShipFee,BTotalPrice) values ('龙仓健','18330386863','燕大','1','留言','圆通','20','20');select @@Identity";
      //  DataBase db = new DataBase();
      //     int orderid= db.RunProc(sql);
        string UserName = Session["UID"].ToString();
        string MbID = db.RunProcReturn("select MemberID from tb_Member where UserName='" + UserName + "'");
        int orderid = ope.InsertOrder(DateTime.Now,strName,strPhone,strAddress,MbID,strRemark,strShipType,strTotalShipFee,strBTotalPrice);
          
            int IntBookID; //商品ID
            int IntNum;    //购买商品数量
            int fltTotalPrice;
        int intMemberID;
        intMemberID=Convert.ToInt32(MbID);
            //对订单中的每一个货物插入订单详细表中
            foreach (GridViewRow gvr in this.gvShopCart.Rows)
            {
                IntBookID = int.Parse(gvr.Cells[1].Text);
                IntNum = int.Parse(gvr.Cells[3].Text);
                fltTotalPrice = int.Parse(gvr.Cells[5].Text);
                ope.InsertDetail(IntBookID,IntNum,orderid,fltTotalPrice,intMemberID,DateTime.Now);
           ...
           // Response.Redirect("PayWay.aspx?OrderID=" + IntOrderID);
            ...
            Response.Write("<script>alert('提交成功订单号为：" + orderid + "');window.location.href='./UserHistory.aspx'</script>");
